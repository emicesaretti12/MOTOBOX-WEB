...
(function () {
  ...
  function initHomePage() {
    ...
    // Show only 3 motos: featured first, then by order
    const featured = motos.filter(m => m.destacada).slice(0, 3);
    const display = featured.length >= 3 ? featured : motos.slice(0, 3);

    display.forEach((moto, idx) => {
