...
(function () {
  ...
  function initHomePage() {
    ...
    motos.forEach((moto, idx) => {
      ...
      const directWaMsg = `Hola Motobox! Quiero más información sobre la ${moto.marca} ${moto.modelo} 0km (${moto.cilindrada}).`;

      card.innerHTML = `
        <div class="simple-card-img">
          ${moto.badge ? `<span class="simple-card-badge">${moto.badge}</span>` : ""}
          <img src="${moto.imagen}" alt="${moto.marca} ${moto.modelo} 0km" loading="lazy">
        </div>
        <div class="simple-card-body">
          <h3 class="simple-card-name">${moto.marca} ${moto.modelo}</h3>
          <p class="simple-card-desc">${moto.tagline}</p>
          <a href="${buildWhatsAppUrl(directWaMsg)}" class="simple-card-btn" target="_blank" rel="noopener">
